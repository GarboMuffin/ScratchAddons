...
const matchesUSB = (gamepad, vendor, product) => {
  // For the same controller, possible values for gamepad.id include:
  // Chrome on Linux: Microsoft Controller (STANDARD GAMEPAD Vendor: 045e Product: 02ea)
  // Firefox on Linux: 045e-02ea-Microsoft X-Box One S pad
  // GNOME Web on Linux: Microsoft X-Box One S pad
 ...
 */
const fixBrokenGamepad = (gamepad) => {
...
const getFixedGamepads = () => navigator.getGamepads().map(fixBrokenGamepad);
...
class GamepadLib extends EventTarget {
  ...
  handleConnect(e) {
    ...
    this.connectCallbacks = [];
    const gamepad = fixBrokenGamepad(e.gamepad);
  ...
  update(time) {
    ...
    const gamepads = getFixedGamepads();
